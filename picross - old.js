...
let seed = 6; // seed 6 in a 10x10 is useful for wildcard testing
let visualizeSolution = true;
...
function mousePressed(event) {
    ...
    if (buttonsPressed.middle) {
        NewGame();
    }
    else if (hoveredTile != null && !selectingTiles) {
...
Tile.prototype.GuessTile = function (Guess) {
    ...
    UpdateColumnNumbers(this.column);
}
...
function CalculateLine(LineTiles, LineNumbers) {
    /*
    * Okay, here's how the line algorithm works so pay attention (there will be a quiz):
    *   We recursively check every possible line permutation
    *   If a permutation has the same line numbers (clues) we were provided then the results are stored in lineData
    *   After all permutations have been completed, check lineData to see if any tiles are only filled or only empty and use that to update the actual tile states
    */
    ...
    CheckTile(LineTiles, LineNumbers, 0, new Array(LineTiles.length), lineData, false, -1, 0, requiredSpace);
 ...
 * ----- OPTOMIZATION -----
 * -CountingGroup: bool - Are we in the middle of counting a group
 * -CurrentGroup: int - Which group are we in the middle of counting
 * -GroupCountedSoFar: int - How far into this group have we counted
 * RequiredSpace: int - The amount of space we have to take up to create a valid permutation
 */
let lineData = [];
function CheckTile(LineTiles, LineNumbers, Index, CurrentPermutation, LineData, CountingGroup, CurrentGroup, GroupCountedSoFar, RequiredSpace) {
    ...
    if (endOfValidPermutation) {
        ...
        const alreadyDetermined = LineTiles[Index] !== -1;

        if (alreadyDetermined) {
            if (LineTiles[Index] == 1) {
                if (CountingGroup) {
                    if (GroupCountedSoFar < LineNumbers[CurrentGroup]) { // If this is supposed to be true, but we've reaced the end of a group, then do nothing
                        CurrentPermutation[Index] = 1;
                        CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, true, CurrentGroup, GroupCountedSoFar + 1, RequiredSpace - 1);
                    }
                }
                else if (CurrentGroup < LineNumbers.length - 1) {
                    CurrentPermutation[Index] = 1;
                    CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, true, CurrentGroup + 1, 1, RequiredSpace - 1);
                }
            }
            else {
                if (CountingGroup) {
                    if (GroupCountedSoFar == LineNumbers[CurrentGroup]) {
                        CurrentPermutation[Index] = 0;
                        CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, false, CurrentGroup, 0, RequiredSpace - 1);
                    }
                }
                else {
                    let availableSpace = LineTiles.length - Index;

                    if (availableSpace > RequiredSpace) { // Only test a false if there is enough space to do so
                        CurrentPermutation[Index] = 0;
                        CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, false, CurrentGroup, 0, RequiredSpace);
                    }
                }
            }
        }
            ...
            if (CountingGroup) {
                if (GroupCountedSoFar < LineNumbers[CurrentGroup]) { // If we haven't reached the end of the group yet then test another true
                    CurrentPermutation[Index] = 1;
                    CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, true, CurrentGroup, GroupCountedSoFar + 1, RequiredSpace - 1);
                }
                else { // If we've reached the end of the group then test a false
                    CurrentPermutation[Index] = 0;
                    CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, false, CurrentGroup, 0, RequiredSpace - 1);
                }
            }
            else {
                if (CurrentGroup == LineNumbers.length - 1) { // If we've already counted all the gorups then just test an empty tile
                    CurrentPermutation[Index] = 0;
                    CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, false, CurrentGroup, 0, RequiredSpace);
                }
                else { // There are more groups to count so test a filled, and if there's enough space test an empty
                    let availableSpace = LineTiles.length - Index;
                    ...
                    // Test true and start the next group
                    CurrentPermutation[Index] = 1;
                    CheckTile(LineTiles, LineNumbers, Index + 1, CurrentPermutation, LineData, true, CurrentGroup + 1, 1, RequiredSpace - 1);
                }
            }
...
function IsGridCorrect(Grid, RowNumbers, ColumnNumbers) {
    let newRowNumbers = [];
...
function OuterTileValid(Grid, Index) {
    return Index < 0 || Index >= Grid.length || Grid[Index] != 1;
}

function NewGame() {
    let boardSizeSeedOffset = tileRows * tileColumns + tileColumns; // This probably isn't necesary but I don't like how the numbers stay similar if you change the board size
    ...
    * 3: If there are conflicts during the solve, invert the state and try again
...
function CreateBoard() {
    ...
    for (let i = 0; i < tileColumns; i++) {
        UpdateColumnNumbers(i);
    }
}

function UpdateRowNumbers(Row) {
...
function UpdateColumnNumbers(Column) {
